...
def main():
    ...
    for k in range(1, 6):  # k becomes 1, 2, 3, ... number
        even_better_draw_circles(rg.Point(-250 + k*80,-250 +k*80), 60-k*10, 1+k, 'green', 1+2*k)
    #even_better_draw_circles(rg.Point(0, 0), 10, 5, "blue", 4)
...
def better_draw_circles(point,radius):
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(radius * k)  # radius, 2*radius, 3*radius, ...
...
def even_better_draw_circles(point,radius, number, color,thickness):
    ...
    for k in range(1, number+1):  # k becomes 1, 2, 3, ... number
